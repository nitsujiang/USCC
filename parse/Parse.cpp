...
Parser::Parser(const char *fileName, std::ostream *errStream,
    ...
    : mCurrToken(Token::Unknown), mFileName(fileName), mFileStream(fileName),
      mErrStream(errStream), mASTStream(ASTStream), mLineNumber(1),
      mColNumber(1), mUnusedIdent(nullptr), mNeedPrintf(false),
      mCheckSemant(true), mOutputSymbols(outputSymbols) {
...
// in the list.
bool Parser::peekIsOneOf(
    const std::initializer_list<Token::Tokens> &list) noexcept {
...
//
// NOTE: You should ONLY use this for terminals that are always a specific text.
// Don't use it for identifier, constant, or string, because you'll have no way
// to get the text.
...
void Parser::reportError(const ParseExcept &except) noexcept {
  ...
  mErrors.push_back(
      std::make_shared<Error>(errStrm.str(), mLineNumber, mColNumber));
}
...
void Parser::reportSemantError(const std::string &msg, int colOverride,
                               int lineOverride) noexcept {
  if (mCheckSemant) {
    int col;
    if (colOverride == -1) {
      col = mColNumber;
...
void Parser::displayErrorMsg(const std::string &line,
                             std::shared_ptr<Error> error) noexcept {
...
// Takes the expression, and if it's a char expression, converts it to an int
// type expression. Otherwise it doesn't do anything.
std::shared_ptr<ASTExpr>
Parser::charToInt(std::shared_ptr<ASTExpr> expr) noexcept {
  // If null, return as is
  if (!expr) {
      return expr;
  }
  ...
  if (expr->getType() == Type::Char) {
      // If it's a constant expression, just change its type
      auto constExpr = std::dynamic_pointer_cast<ASTConstantExpr>(expr);
      if (constExpr) {
          constExpr->changeToInt();
          return constExpr;
      }
      // Otherwise, convert to ASTToIntExpr
...
std::shared_ptr<ASTExpr>
Parser::intToChar(std::shared_ptr<ASTExpr> expr) noexcept {
  // If null, return as is
  if (!expr) {
      return expr;
  }
  // If already char, return as is
  if (expr->getType() == Type::Char) {
      return expr;
  }
  // If int
  if (expr->getType() == Type::Int) {
      ...
      if (toIntExpr) {
          return toIntExpr->getChild();
      }
      // Otherwise, convert to ASTToCharExpr
...
shared_ptr<ASTFunction> Parser::parseFunction() {
  ...
  if (peekIsOneOf({Token::Key_void, Token::Key_int, Token::Key_char})) {
    ...
    if (peekAndConsume(Token::LBracket)) {
      reportSemantError("USC does not allow return of array types",
                        mColNumber - 1);
