 ...
 : mCurrScope(nullptr)
{
	...
	auto funcDummy = createIdentifier("@@function");
	funcDummy->setType(Type::Function);

	auto varDummy = createIdentifier("@@variable");
	varDummy->setType(Type::Int);

	auto printfDummy = createIdentifier("printf");
	printfDummy->setType(Type::Function);
}

SymbolTable::~SymbolTable() noexcept
{
	while (mCurrScope != nullptr && mCurrScope->getParent() != nullptr)
	{
		exitScope();
	}
	if (mCurrScope) {
...
{
	Identifier* ident = mCurrScope->search(name);
	if (ident) {
		return ident;
	}
	return nullptr;
...
{
	ScopeTable* newScope = new ScopeTable(mCurrScope);
	mCurrScope = newScope;
	return newScope;
}
...
: mParent(parent)
{
	// Simply add this to the parent's list of children, if any
	if (mParent) {
...
{
	// Delete all the identifiers in this scope
...
{
	// Allow higher level code to prevent duplicates and errors
